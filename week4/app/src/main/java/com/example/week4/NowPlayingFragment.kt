...
class NowPlayingFragment(private val gridLayout : Boolean, var db: AppDatabase) : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        if(gridLayout) {
            adapter = MovieAdapter(ctx = activity as Context, movies = MovieModel.getDataFromApiForNowPlaying(), type = 1, database = db)
            rvNowPlayingFragment.layoutManager = GridLayoutManager(activity,3)
        } else {
            adapter = MovieAdapter(ctx = activity as Context, movies = MovieModel.getDataFromApiForNowPlaying(), type = 0, database = db)
